...
async function createCategory(req,res) {
    try {
     sqlQuery = 'INSERT INTO categories (name_uz,name_ru,desc_short_uz,desc_short_ru,images,view_count,parent_id) VALUES(?,?,?,?,?,?,?)'
    const { name_uz,name_ru,desc_short_uz,desc_short_ru,image,view_count,parent_id } = req.body
    db.query(sqlQuery, [name_uz,name_ru,desc_short_uz,desc_short_ru,image,view_count,parent_id])
    res.send("ishladi")
    } catch (error) {
...
async function updateCategory(req, res) {
    try {
      ...
      const {name_uz, name_ru, desc_short_uz, desc_short_ru, images, view_count, parent_id} = req.body;
      const updateSql = "UPDATE categories SET  name_uz = ?, name_ru = ?, desc_short_uz = ?, desc_short_ru = ?, images  = ?, view_count = ?, parent_id = ?  WHERE id = ?";
      
      await db.query(updateSql, [name_uz, name_ru, desc_short_uz, desc_short_ru, images, view_count, parent_id, id]);
